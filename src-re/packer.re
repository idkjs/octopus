module Bytes = {
  ...
  [@noalloc]
  external unsafe_int_of_bits: (bytes, int, int) => int = "stub_int_of_bits";

  let int8_of_bits = (buf, pos) => {
    if (Bytes.length(buf) < pos + 1) {
  ...
  [@noalloc]
  external unsafe_blit_i8: (bytes, int, int) => unit = "stub_blit_i8";
  [@noalloc]
  external unsafe_blit_i16: (bytes, int, int) => unit = "stub_blit_i16";
  [@noalloc]
  external unsafe_blit_i32: (bytes, int, int) => unit = "stub_blit_i32";
  [@noalloc]
  external unsafe_blit_i64: (bytes, int, Int64.t) => unit = "stub_blit_i64";
  [@noalloc]
  external unsafe_blit_varint: (bytes, int, int) => int = "stub_blit_varint";

  [@noalloc]
  external unsafe_blit_field_i8: (bytes, int, int) => int =
    "stub_blit_field_i8";
  [@noalloc]
  external unsafe_blit_field_i16: (bytes, int, int) => int =
    "stub_blit_field_i16";
  [@noalloc]
  external unsafe_blit_field_i32: (bytes, int, int) => int =
    "stub_blit_field_i32";
  [@noalloc]
  external unsafe_blit_field_i64: (bytes, int, Int64.t) => int =
    "stub_blit_field_i64";
  [@noalloc]
  external unsafe_blit_field_bytes: (bytes, int, bytes) => int =
    "stub_blit_field_bytes";

  let blit_i8 = (str, pos, n) => {
    if (Bytes.length(str) < pos + 1) {
      invalid_arg("blit_i8");
    };
  ...
  let blit_i16 = (str, pos, n) => {
    if (Bytes.length(str) < pos + 2) {
      invalid_arg("blit_i16");
    };
  ...
  let blit_i32 = (str, pos, n) => {
    if (Bytes.length(str) < pos + 4) {
      invalid_arg("blit_i32");
    };
    unsafe_blit_i32(str, pos, n);
  };

  let blit_i64 = (str, pos, n) => {
    if (Bytes.length(str) < pos + 8) {
      invalid_arg("blit_i64");
    };
    unsafe_blit_i64(str, pos, n);
  };

  let blit_varint32 = (str, pos, n) => {
    if (Bytes.length(str) < pos + 5) {
      invalid_arg("blit_ber");
    };
    unsafe_blit_varint(str, pos, n);
  };

  [@noalloc]
  external varint32_size: ([@untagged] int) => [@untagged] int =
    "abort" "varint32_sizeof";
};

type t = {
  mutable buf: bytes,
  mutable used: int,
}; /* stub_box_dispatch depends on layout of t */
...
let contents = pa =>
  if (Bytes.length(pa.buf) == pa.used) {
    let v = pa.buf;
...
let need = (pa, n) => {
  if (Bytes.length(pa.buf) - pa.used < n) {
    let size = ref(max(16, Bytes.length(pa.buf)));
    while (size^ - pa.used < n) {
      size := size^ * 2;
    };
    let buf' = Bytes.extend(pa.buf, 0, size^ - Bytes.length(pa.buf));
    pa.buf = buf';
  };
  let ret = pa.used;
  pa.used = pa.used + n;
...
let blit = ({buf}, offt, s) =>
  Bytes.blit(s, 0, buf, offt, Bytes.length(s));
...
let hexdump = ({buf, used}) =>
  if (used == 0) {
    ...
    let str = Buffer.create(100);
    for (i in 0 to used - 2) {
